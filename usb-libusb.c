...
if (0)
	...
	ret = usb_interrupt_write(bl->h, USB_ENDPOINT_OUT | 1, bl->buf.char_arr, len, 5000);
	if (ret < 0) {
		errno = ret = -ret;
	...
	if (do_read) {
		ret = usb_interrupt_read(bl->h, USB_ENDPOINT_IN | 1, bl->buf.char_arr, 64, 5000);
		if (ret < 0) {
			errno = ret = -ret;
...
#if 0
static int compare_mem_regions(const void *a, const void *b) {
	...
	return aa - bb;
#else
	/* This is better if we could inline it via -findirect-inlining */
	if (aa > bb)
		return 1;
	else if (aa < bb)
		return -1;
...
int bl_query(struct usb_hid_bootloader *bl) {
	...
	/* Sort memory regions by address */
	//qsort(bl->mem, bl->mem_region_count, sizeof(*bl->mem), compare_mem_regions);

	return ret;
}

static int get_region_type(struct usb_hid_bootloader *bl, uint32_t addr, uint32_t len)
{
...
static int bl_flush(struct usb_hid_bootloader *bl, bool simulate_only)
{
	 ...
	 *
	 * I'm too lazy to do this while accumulating data, so we'll do it here
	 * if we have to.  */
	if (offset) {
		unsigned i;
